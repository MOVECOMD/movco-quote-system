...
export default function AdminPortalPage() {
  ...
  const [currentAdmin, setCurrentAdmin] = useState<{ id: string; email: string; name: string; role: string } | null>(null);
  const [adminUsers, setAdminUsers] = useState<{ id: string; email: string; name: string; role: string; created_at: string }[]>([]);
  ...
  const navItems = [
    ...
    { id: 'divider4', type: 'divider' as const, label: 'SETTINGS' },
        ...
        {page === 'billing' && (
          <div>
            ...
            <div className="bg-white rounded-xl p-6 border border-gray-200 mb-5">
              ...
              {[
                { plan: 'CRM Pro', price: '£149.99/mo', count: crmPartners.filter((p) => p.active && (p._mrr || 0) > 0).length, mrr: crmPartners.reduce((s, p) => s + (p._mrr || 0), 0), color: '#7C3AED' },
                { plan: 'Calculator', price: '£99.99/mo', count: [...storagePartners, ...calcPartners].filter((p) => (p._mrr || 0) > 0).length, mrr: [...storagePartners, ...calcPartners].reduce((s, p) => s + (p._mrr || 0), 0), color: c.accent },
                { plan: 'Trial', price: 'Free', count: allPartners.filter((p) => !p._mrr).length, mrr: 0, color: c.amber },
              ].map((row) => (
