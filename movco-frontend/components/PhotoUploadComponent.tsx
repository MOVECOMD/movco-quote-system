...
export default function PhotoUploadComponent() {
  ...
  // Refs for file inputs so we can reset them after each use
  ...
  const uploadPhotos = async (roomId: string, files: FileList, inputRef?: HTMLInputElement | null) => {
    if (!files || files.length === 0) return;

    // Use functional state update to avoid stale closure
    setRooms((prev) =>
      prev.map((r) => (r.id === roomId ? { ...r, uploading: true } : r))
    );
    ...
    try {
      ...
      // Use functional state update — this guarantees we append to the LATEST state
      setRooms((prev) =>
        prev.map((r) =>
          r.id === roomId
      ...
      setRooms((prev) =>
        prev.map((r) => (r.id === roomId ? { ...r, uploading: false } : r))
      );
    }

    // Reset the input so the same file or a new camera photo can be selected again
    if (inputRef) {
      inputRef.value = '';
    }
  };
          ...
          <div className="space-y-4">
            {rooms.map((room) => (
              <div key={room.id} className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg p-6 border border-white/20 transition-all duration-200 hover:shadow-xl">
                ...
                {/* Upload area — two buttons: Take Photo (camera) + Upload Photos (gallery) */}
                {room.uploading ? (
                  <div className="border-2 border-dashed border-blue-300 rounded-xl p-6 text-center bg-blue-50/50 mb-4">
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    {/* TAKE PHOTO — opens camera directly, one photo at a time, accumulates */}
                    <label className="block cursor-pointer">
                      <div className="border-2 border-dashed border-slate-200 rounded-xl p-5 text-center hover:border-blue-400 hover:bg-blue-50/50 transition-all duration-200">
                        <div className="w-11 h-11 bg-blue-100 rounded-full flex items-center justify-center mx-auto mb-2">
                          <svg className="w-6 h-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      <input
                        ref={(el) => { cameraInputRefs.current[room.id] = el; }}
                        type="file"
                        accept="image/*"
                        capture="environment"
                        onChange={(e) => {
                          if (e.target.files) {
                            uploadPhotos(room.id, e.target.files, e.target);
                          }
                        }}
                        className="hidden"
                      />
                    ...
                    {/* UPLOAD FROM GALLERY — allows multiple selection */}
                    <label className="block cursor-pointer">
                      <div className="border-2 border-dashed border-slate-200 rounded-xl p-5 text-center hover:border-green-400 hover:bg-green-50/50 transition-all duration-200">
                        <div className="w-11 h-11 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-2">
                          <svg className="w-6 h-6 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                          </svg>
                        </div>
                        <p className="text-slate-700 font-medium text-sm">Upload Photos</p>
                        <p className="text-xs text-slate-400 mt-0.5">Choose from gallery</p>
                      </div>
                      <input
                        ref={(el) => { galleryInputRefs.current[room.id] = el; }}
                        type="file"
                        accept="image/*"
                        multiple
                        onChange={(e) => {
                ...
                {room.photos.length > 0 && (
                  <div>
                    <p className="text-xs text-slate-500 mb-2 font-medium">{room.photos.length} photo{room.photos.length !== 1 ? 's' : ''}</p>
