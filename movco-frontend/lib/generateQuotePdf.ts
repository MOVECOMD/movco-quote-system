...
export async function downloadQuotePdf(data: QuotePdfData, filename?: string) {
  ...
  const checkNewPage = (requiredSpace: number) => {
    if (y + requiredSpace > pageHeight - 30) {
      ...
      doc.text(displayName.toUpperCase(), margin, 8);
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(7);
      doc.text('QUOTE CONTINUED', pageWidth - margin, 8, { align: 'right' });
      y = 22;
    }
  };
  ...
  if (data.items && data.items.length > 0) {
    ...
    autoTable(doc, {
      ...
      didDrawPage: () => {
        // Repeat compact header on overflow pages
        doc.setFillColor(...primaryColor);
        doc.rect(0, 0, pageWidth, 12, 'F');
        doc.setTextColor(255, 255, 255);
        doc.setFontSize(8);
  ...
  if (branding.payment_terms || branding.deposit_amount) {
    const boxHeight = branding.deposit_amount ? 20 : 12;
    checkNewPage(boxHeight + 12);
    doc.setFillColor(249, 250, 251);
    doc.roundedRect(pageWidth - margin - 80, y, 80, boxHeight, 2, 2, 'F');
    doc.setFontSize(8);
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(...grayColor);
    if (branding.payment_terms) {
      doc.text(branding.payment_terms, pageWidth - margin - 40, y + 7, { align: 'center' });
    }
    if (branding.deposit_amount) {
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(...primaryColor);
      doc.text(`Deposit: ${branding.deposit_amount}`, pageWidth - margin - 40, y + 16, { align: 'center' });
    }
    y += boxHeight + 8;
  }
  ...
  if (branding.terms_text) {
    ...
    // Render terms line by line, adding pages as needed
    for (let i = 0; i < splitTerms.length; i++) {
      ...
      doc.text(splitTerms[i], margin, y);
      y += 4;
    }
    y += 6;
  }
  ...
  if (branding.bank_details) {
    ...
    checkNewPage(bankLines.length * 4.5 + 20);
    doc.setFontSize(8);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(...primaryColor);
  ...
  // ===== WATERMARK =====
  if (branding.watermark_draft !== false && (!data.status || data.status === 'draft' || data.status === 'pending')) {
    for (let p = 1; p <= totalPages; p++) {
      doc.setPage(p);
      doc.setFontSize(80);
      doc.setFont('helvetica', 'bold');
