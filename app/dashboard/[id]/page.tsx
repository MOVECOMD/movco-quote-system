    ...
    <main className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 p-6">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <section className="bg-white rounded-2xl shadow-lg p-6 space-y-4 border border-white/20">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-slate-800 mb-1"><span className="font-semibold">From:</span> {quote.starting_address}</p>
              <p className="text-sm text-slate-800"><span className="font-semibold">To:</span> {quote.ending_address}</p>
            </div>
            <div>
        ...
        <section className="bg-white rounded-2xl shadow-lg p-6 space-y-4 border border-white/20">
          ...
          <p className="text-4xl font-bold text-green-700">£{estimate.toLocaleString("en-GB", { maximumFractionDigits: 0 })}</p>
          ...
          <div className="flex flex-wrap gap-6 text-sm text-slate-800 mt-4 pt-4 border-t border-slate-200">
            <div><span className="font-semibold text-slate-900">Estimated volume:</span> {totalVolumeM3} m³</div>
            <div><span className="font-semibold text-slate-900">Estimated area:</span> {totalAreaM2} m²</div>
          </div>
        </section>

        <section className="bg-white rounded-2xl shadow-lg p-6 space-y-4 border border-white/20">
          <h2 className="text-lg font-semibold text-slate-900">✨ AI-Detected Inventory</h2>
          <p className="text-xs text-slate-700 mb-3">Furniture and items detected by Claude Vision AI from your uploaded photos.</p>
              ...
              <tbody>
                {items.map((item, idx) => (
                  <tr key={idx} className="border-t border-slate-200">
                    ...
                    <td className="px-4 py-3 text-xs text-slate-700">{item.note ?? "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
